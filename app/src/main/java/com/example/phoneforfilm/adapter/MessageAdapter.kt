...
R.id.menu_change_language   -> {
    ...
    activity?.let { act ->
        val langs = arrayOf("en","nl")
        val names = arrayOf(act.getString(R.string.english), act.getString(R.string.dutch))
        androidx.appcompat.app.AlertDialog.Builder(act)
            .setTitle(R.string.choose_language)
            .setItems(names) { _, which ->
                com.example.phoneforfilm.utils.LocaleManager.setLocale(act, java.util.Locale(langs[which]))
                act.recreate()
            }
            .show()
    }
...
R.id.menu_change_theme      -> {
    val activity = anchor.context as? android.app.Activity
    activity?.let { com.example.phoneforfilm.utils.ThemeManager.showThemeChooser(it) }
}
